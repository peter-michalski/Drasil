...
import Language.Drasil.Choices (DesignChoice(..), designChoiceStr)
...
getInOutCall n inFunc outFunc = do
  ...
  let getInOutCall' Nothing = return Nothing
      getInOutCall' (Just m) = do
        ...
        stmt <- fAppInOut m (designChoiceStr n) (map valueOf ins) outs both
        return $ Just stmt
  getInOutCall' mm
...
-- current class.
getCall :: (Either Name DesignChoice) -> GenState (Maybe Name)
getCall (Left n) = do
  g <- get
...
getCall (Right n) = do
  g <- get
  let currc = currentClass g
      getCallExported Nothing = getCallInClass (Map.lookup (designChoiceStr n) $ clsMap g)
      getCallExported m = return m
      getCallInClass Nothing = return Nothing
      getCallInClass (Just c) = if c == currc then return $ Map.lookup c (eMap 
        g) <|> error (c ++ " class missing from export map")
        else return Nothing
  getCallExported $ Map.lookup (designChoiceStr n) (eMap g)