...
transMot = eqModel transMotQD [qw velI, qw time, qw gravitationalAccel, qw forceI, qw massI] 
  [sy velI $> 0, sy time $> 0, sy gravitationalAccel $> 0, sy forceI $> 0, sy massI $> 0 ]
    (qw accI) [] [] Nothing "transMot"
  [transMotDesc, transMotOutputs, rigidTwoDAssump, noDampConsAssumps]

transMotQD :: QDefinition
transMotQD = fromEqn' "transMotQD" transMotNP EmptyS (eqSymb accI) Real transMotEqn
...
transMotEqn :: Expr
transMotEqn = deriv (apply1 velI time) time
...
rotMot = eqModel rotMotQD [qw angularVelocity, qw time, qw torqueI, qw momentOfInertia]
  [sy angularVelocity $> 0, sy time $> 0, sy torqueI $> 0, sy momentOfInertia $> 0] 
    (qw angularAccel) [sy angularAccel $> 0] [] Nothing "rotMot"
  [rotMotDesc, rigidTwoDAssump, rightHandAssump]

rotMotQD :: QDefinition
rotMotQD = fromEqn' "rotMotQD" rotMotNP EmptyS (eqSymb angularAccel) Real rotMotEqn
...
-- FIXME: should this "equation" be a series of equations?
rotMotEqn :: Expr
rotMotEqn = deriv (apply1 angularVelocity time) time
